"""
BTC techninės analizės signalų botas -> Telegram (išplėsta versija)
----------------------------------------------------------------------
Kas valandą tikrina BTC/USD kainą (Kraken duomenys) ir skaičiuoja 8
...
STATE_FILE = "ta_state_btc.json"
CHART_FILE = "btc_chart.png"
...
CHART_IMG_FILE = "tv_chart_btc.png"
...
def create_chart(df, rsi, macd_line, signal_line, bb_upper, bb_mid, bb_lower,
    ...
    ax1 = axes[0]
    ax1.plot(x, df_plot["close"], color="#38BDF8", linewidth=1.3, label="BTC kaina")
    ...
    ax1.set_title("BTC/USD - 1h grafikas", color="#E2E8F0", fontsize=12, loc="left")
...
def main():
    ...
    try:
        short_caption = f"🔍 BTC signalas (1h) - ${current_price:,.2f}\n" + " | ".join(
    ...
    message = f"<b>🔍 BTC techninės analizės signalas (1h)</b>\n\nKaina: ${current_price:,.2f}\n\n"
    ...
    message += (
        f"\n<b>📈 Gyvas grafikas:</b> "
        f"<a href=\"https://www.tradingview.com/chart/?symbol=KRAKEN:XBTUSD\">Atidaryti TradingView</a>\n"
    )
    message += (
